	...
	if(isset($_GET['comparison_grp'])){
		...
		for($idx = 0; $idx < count($comparison_values); $idx++) {
			...
			for ($i=0; $i < count($conditions); $i++) { 
				...
				$sql = "SELECT COUNT(*) as count FROM (SELECT * FROM $table_name NATURAL JOIN teacher_to_courses WHERE $conditions[$i] and $course_id_sql_condition and $teacher_id_sql_condition  and seat_no like \"_$batch%\") a JOIN students as b on a.seat_no LIKE CONCAT('%', b.seat_no ,'%') where gender $gender_sql_symbol;";
